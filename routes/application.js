...
module.exports = function(emitter){
  ...
  router.post('/enable', function(req, res){
    let redirect_uri = config.serverurl + '/eloqua/lifecycle/enable/callback/' + req.query.InstallId;
  ...
  router.get('/callback/:InstallId', function(req, res){
    let redirect_uri = config.serverurl + '/eloqua/lifecycle/enable/callback/' + req.params.InstallId;
    let callback_option = {
    ...
    enable_uri.then(function(enable_uri_res){
      ...
      let upsert = emitter.invokeHook('db::upsert', queryOption);
      upsert.then(function(upsert_res){
        res.redirect(upsert_res.CallbackUrl);
      },function(err){
        res.status(400).json(err);
      })
    },function(err){
